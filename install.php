...
/*  Ora cancello la cartella dove era memorizzato il db perche non serve piu   */
if(!is_dir('db')) {echo "Cannot find db directory! </br>";}
else{
  echo "Deleting db folder...</br>";
  deleteDirectory('db');
  if(is_dir('db')) echo "FAILED deleting db folder, please remove it by hand;</br>";
  else echo "SUCCESS deleting db folder.</br>";
}


/*  Ora l-autodistruzione del file di istallazione  */
echo "Deleting self...</br>";
unlink('install.php');  /*  Anche se faccio l'unlink, lo script viene comunque eseguito fino alla fine  */
...
/*  Ora non mi serve piu $originalIndex perche il suo contenuto sta dentro index  */
unlink($originalIndex);
