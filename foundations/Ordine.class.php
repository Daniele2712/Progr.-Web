...
class Ordine extends Foundation{
    ...
    public static function save(Model $ordine){
        Pagamento::save($ordine->getPagamento());
        Indirizzo::save($ordine->getIndirizzo());
        DatiAnagrafici::save($ordine->getDatiAnagrafici());

        //salvo anche il magazzino perchè è cambiato il numero di prodotti disponibili con l'ordine
        Magazzino::save($ordine->getMagazzino());
    ...
    public static function create(array $row): Model{
        $indirizzo = Indirizzo::find($row['id_indirizzo']);
        $datiAnagrafici = DatiAnagrafici::find($row['id_dati_anagrafici']);
        $magazzino = Magazzino::find($row["id_magazzino"]);
        ...
        while($riga = $ris->fetch_array(MYSQLI_ASSOC)){
            ...
            $prezzo=$riga['prezzo'];
            $money=new \Models\Money($prezzo, $row['id_valuta']);
            $quant=$riga['quantita'];
        ...
        $ordine = new \Models\Ordine($orderId, $items, $indirizzo, $datiAnagrafici, $magazzino, $subtotale, $spese_spedizione, $totale, $id_valuta, $data_ordine, $data_consegna, $stato);    // aggiungere anche gli altri campi , e poi manca la parte del pagamento....
       return $ordine;
    }
